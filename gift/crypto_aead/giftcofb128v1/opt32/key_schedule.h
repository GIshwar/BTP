...
#include <stdint.h>  // Ensures uint32_t is recognized

// Ensure SWAPMOVE is defined only if not already present
#ifndef SWAPMOVE
#define SWAPMOVE(x, y, mask, n) ((x) ^= (((y) ^ (x)) & (mask)) << (n), (y) ^= (((y) ^ (x)) & (mask)) >> (n))
#endif

// Ensure ROR (Rotate Right) is defined if not already
#ifndef ROR
